...
- **`mo-home.jsp`** — Shell layout: sidebar (includes applicants **unread badge** `navApplicantsBadge`), top bar, welcome card, and four route sections (dashboard, jobs, applicants, profile). Reuses shared TA layout/component styles plus MO-specific `assets/mo/css/mo-home.css`. Loads MO JS modules in order: `settings`, `route-nav`, `modal`, `job-board`, `applicants`, `dashboard`, `profile`, then `mo-home.js` bootstrap.
